...
const EventTracker = () => {
  ...
  const handleFilter = (filterType) => {
    switch (filterType) {
      ...
      case "past":
        setEvents(
          events.filter((event) => moment(event.start).isBefore(new Date()))
        );
        break;
      case "upcoming":
        setEvents(
          events.filter((event) => moment(event.start).isAfter(new Date()))
        );
        break;
      default:
        break;
  ...
  return (
    <div>
      ...
      {showCreateEvent && (
        <Popup title="Create Event" onClose={() => setShowCreateEvent(false)}>
          <form>
            <label>
              Title:
              <input type="text" />
      ...
      {showEditEvent && (
        <Popup title="Edit Event" onClose={() => setShowEditEvent(false)}>
          <form>
            <label>
              Title:
            ...
            <button
              onClick={(title, start, end) =>
                handleEditEvent(title, start, end)
              }
            >
              Edit
            </button>
